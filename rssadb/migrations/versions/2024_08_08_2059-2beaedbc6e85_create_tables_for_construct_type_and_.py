...
def upgrade() -> None:
    op.create_table('construct_type',
        sa.Column('id', UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
        sa.Column('type', sa.String, nullable=False),
        sa.Column('enabled', sa.Boolean, nullable=False)
    )

    op.create_table('construct_scale',
        sa.Column('id', UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
    ...
    op.add_column('survey_construct', 
        sa.Column('type', UUID(as_uuid=True), nullable=True)
    )

    op.create_foreign_key('fk_survey_construct_type', 'survey_construct', 
        'construct_type', ['type'], ['id'])

    op.add_column('survey_construct',
        sa.Column('scale', UUID(as_uuid=True), nullable=True)
    )

    op.create_foreign_key('fk_survey_construct_scale', 'survey_construct',
        'construct_scale', ['scale'], ['id'])
    
    op.add_column('construct_item',
