...
down_revision: Union[str, None] = "dd1d4ec486f5"
...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "participant_response_pkey", "participant_survey_response", type_="primary"
    )
    op.drop_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "discarded", sa.Boolean(), nullable=False, server_default=sa.text("FALSE")
        ),
    ...
    with op.batch_alter_table("survey_item_response", schema=None) as batch_op:
        batch_op.alter_column(
            "item_id", existing_type=UUID(as_uuid=True), nullable=False
        )
        batch_op.alter_column(
            "construct_id", existing_type=UUID(as_uuid=True), nullable=False
        )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("survey_item_response", schema=None) as batch_op:
        batch_op.drop_constraint(
            op.f("fk_survey_item_response_study_step_id"), type_="foreignkey"
        )
    ...
    op.create_primary_key(
        "participant_response_pkey", "participant_survey_response", ["id"]
    )
