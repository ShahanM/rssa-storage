...
from .survey_components import ConstructScale, SurveyConstruct
...
class PageContent(Base):
    ...
    page_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("step_page.id"), primary_key=True
    )
    content_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("survey_construct.id"), primary_key=True
    )
    scale_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("construct_scale.id"), primary_key=True
    )
    order_position: Mapped[int] = mapped_column()
    enabled: Mapped[bool] = mapped_column(default=True)
    ...
    survey_construct: Mapped["SurveyConstruct"] = relationship(
        "SurveyConstruct", back_populates="page_contents"
    )
    construct_scale: Mapped["ConstructScale"] = relationship(
        "ConstructScale", back_populates="page_contents"
    )
...
class Page(Base):
    ...
    step_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("study_step.id"), nullable=False
    )
