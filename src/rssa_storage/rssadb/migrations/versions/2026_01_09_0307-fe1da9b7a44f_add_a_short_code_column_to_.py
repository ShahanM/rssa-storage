...
def upgrade() -> None:
    ...
    for row in results:
        ...
        connection.execute(sa.text("UPDATE study_conditions SET short_code = :code WHERE id = :id"),
            {"code": new_code, "id": row.id}
        )
    op.alter_column('study_conditions', 'short_code', nullable=False)
    op.create_unique_constraint(op.f('uq_study_conditions_study_id_short_code'), 'study_conditions', ['study_id', 'short_code'], deferrable=True, initially='DEFERRED')
